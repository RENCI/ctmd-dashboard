...
exports.proposalsByTic = (req, res) => {
  ...
  const colorScale = d3
    ...
    .range(["#3c9399", "#857a84", "#98bbbb", "#cdb4cb", "#161516"]);
  ...
  getProposals()
    .then((proposals) => {
      ...
      const xScale = d3
        ...
        .domain([
          0,
          d3.max(tics, (d) => d3.sum(d.values, (d) => d.values.length)),
        ])
        .range([0, innerWidth]);
      ...
      tics.forEach((d) => {
        ...
        d.values.sort((a, b) => {
          return d3.ascending(
            colorScale.domain().indexOf(a.key),
            colorScale.domain().indexOf(b.key)
          );
        });
      ...
      const item = svg
        ...
        .attr(
          "transform",
          "translate(" +
            (width - margin.right - legendYScale.bandwidth()) +
            "," +
            margin.top +
            ")"
        )
